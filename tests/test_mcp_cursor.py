...
def test_invalid_config_is_not_replaced(tmp_path, capsys):
    path = tmp_path / "mcp.json"
    path.write_text("{not-json")
    assert mcp_cmd.cmd_mcp_install(_args(path)) == 1
    assert path.read_text() == "{not-json"
    assert "cannot read Cursor MCP config" in capsys.readouterr().err


def test_install_rejects_non_utf8_config_without_rewriting(tmp_path, capsys):
    path = tmp_path / "mcp.json"
    original = b'{"mcpServers": {"other": "\xff"}}'
...
def test_uninstall_rejects_non_utf8_config_without_rewriting(tmp_path, capsys):
    path = tmp_path / "mcp.json"
    original = b'{"mcpServers": {"fidelis": "\xff"}}'
    path.write_bytes(original)
